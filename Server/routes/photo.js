                    ...
                    if(err){
                        ...
                        fs.write(fd,resp.body,0,resp.body.length,function(e){
                            if(e){
                                ...
                                res.statusCode=200;
                                var rstURL = 'http://'+serverIP+':'+port+"/image/manul/"+fileName;
                                res.end(rstURL);
                            }
                        });
                    }
                ...
                if (files.size<1) {
                    ...
                    for(var i=0;i<files.length;i++){
                        if((i-judge)>=0&&(i-judge)<=9){
                            var fileName = __dirname+"/../static/upload/"+files[i];
                            var time = fs.statSync(fileName).ctime;
                            var element = {
                                time: time,
                                url: 'http://'+serverIP+':'+port+'/image/upload/'+files[i]
                            }
                            rst.push(element)
                        }
                    }
